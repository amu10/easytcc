...
public class EasyTccAutoConfiguration {
    ...
    @Bean @ConditionalOnMissingBean @ConditionalOnProperty(prefix = "easy-tcc", name = "storage", havingValue = "redis")
    public TransactionRepository easyTccRedisRepository(EasyTccProperties p) { return new RedisTransactionRepository(p.getRedisUrl()); }
    ...
    public RecoveryScheduler easyTccRecoveryScheduler(TransactionRepository r, TransactionManager m, EasyTccProperties p) {
        return new RecoveryScheduler(r, m, p.getRecoveryInterval(), p.getRecoveryBatchSize(), p.getPurgeRetentionMillis());
    }
    ...
    @Configuration @ConditionalOnClass(name = "org.springframework.boot.actuate.health.HealthIndicator")
    static class ActuatorConfiguration {
        @Bean @ConditionalOnMissingBean EasyTccHealthIndicator easyTccHealthIndicator(TransactionRepository r){return new EasyTccHealthIndicator(r);}
    }
    @Configuration @ConditionalOnClass(name = "io.micrometer.core.instrument.MeterRegistry")
    static class MetricsConfiguration {
        @Bean @ConditionalOnMissingBean EasyTccMeterBinder easyTccMeterBinder(TransactionManager m,TransactionRepository r){return new EasyTccMeterBinder(m,r);}
    }
}
