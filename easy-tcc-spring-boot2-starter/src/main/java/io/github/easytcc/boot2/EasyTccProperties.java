...
public class EasyTccProperties {
    ...
    public long getPurgeRetentionMillis() { return purgeRetentionMillis; }
    public void setPurgeRetentionMillis(long purgeRetentionMillis) { this.purgeRetentionMillis = purgeRetentionMillis; }

    public void validate() {
        if (maxRetries < 0) throw new IllegalArgumentException("easy-tcc.max-retries must be >= 0");
        if (recoveryInterval <= 0) throw new IllegalArgumentException("easy-tcc.recovery-interval must be > 0");
        if (recoveryBatchSize <= 0) throw new IllegalArgumentException("easy-tcc.recovery-batch-size must be > 0");
        if (purgeRetentionMillis < 0) throw new IllegalArgumentException("easy-tcc.purge-retention must be >= 0");
        if (storage == null || !(storage.equals("file") || storage.equals("jdbc") || storage.equals("redis")))
