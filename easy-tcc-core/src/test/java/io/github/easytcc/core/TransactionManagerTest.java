...
class TransactionManagerTest {
    ...
    @Test void manualRetryPreservesDurableDecisionAndWritesAudit() {
        MemoryRepository repository = new MemoryRepository();
        List<String> calls = new ArrayList<String>();
        TransactionManager manager = new TransactionManager(repository, new RecordingInvoker(calls), 3);
        ...
        assertEquals(GlobalStatus.CONFIRMED, repository.find(tx.getXid()).orElseThrow(AssertionError::new).getStatus());
        assertTrue(repository.findAudit(tx.getXid(), 10).stream().anyMatch(e -> "MANUAL_RETRY".equals(e.getOperation())));
    }

    @Test void purgeCompletedDeletesTerminalTransactionsPastRetention() {
        MemoryRepository repository = new MemoryRepository();
        TransactionManager manager = new TransactionManager(repository, new RecordingInvoker(new ArrayList<String>()), 3);
    ...
    @Test void purgeCompletedSkipsWhenDisabledOrEmpty() {
        MemoryRepository repository = new MemoryRepository();
        TransactionManager manager = new TransactionManager(repository, new RecordingInvoker(new ArrayList<String>()), 3);
    ...
    static final class MemoryRepository implements TransactionRepository {
        ...
        public void appendAudit(TransactionAuditEvent event) { audit.add(event); }
        public List<TransactionAuditEvent> findAudit(String xid, int limit) {
            ...
            for (TransactionAuditEvent event : audit) if (event.getXid().equals(xid) && result.size() < limit) result.add(event);
            return result;
        }
        ...
        public List<String> deleteTerminal(long createdBefore, int limit) {
            ...
            java.util.Iterator<Map.Entry<String, GlobalTransaction>> it = data.entrySet().iterator();
            while (it.hasNext() && deleted.size() < limit) {
                GlobalTransaction tx = it.next().getValue();
                if ((tx.getStatus() == GlobalStatus.CONFIRMED || tx.getStatus() == GlobalStatus.CANCELLED) && tx.getCreatedAt() < createdBefore) {
                    it.remove();
