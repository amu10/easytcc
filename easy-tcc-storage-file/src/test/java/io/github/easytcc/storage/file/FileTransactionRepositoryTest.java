...
class FileTransactionRepositoryTest {
    ...
    @Test
    void persistsAndReloadsTransaction() {
        ...
        assertEquals(
                ...
                repository.find("test-xid").orElseThrow(AssertionError::new).getStatus());
    }

    @Test
    void compareAndSetRejectsStaleWriteAndRecoveryClaimIsExclusive() {
        FileTransactionRepository repository = new FileTransactionRepository(directory);
        ...
        assertTrue(
                repository
                        .tryClaimRecovery("cas-1", recoverable.getVersion(), "node-a", 100L, 10L)
                        .isPresent());
        assertFalse(
                repository
                        .tryClaimRecovery("cas-1", recoverable.getVersion(), "node-b", 100L, 10L)
                        .isPresent());
    }

    @Test
    void deletesTerminalTransactionsPastRetentionOnly() {
        FileTransactionRepository repository = new FileTransactionRepository(directory);
        ...
        assertEquals(
                java.util.Collections.singletonList("old-done"),
                repository.deleteTerminal(1000L, 100));
