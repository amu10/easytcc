...
public final class SpringBranchInvoker implements BranchInvoker {
    ...
    @Override public void validate(BranchTransaction branch) {
    ...
    private void invoke(BranchTransaction branch, String methodName) throws Exception {
        ...
        try { method.invoke(bean, branch.getArguments()); }
            ...
            if (previousXid == null) EasyTccContext.clear(); else EasyTccContext.bind(previousXid);
            if (previousBranch == null) EasyTccContext.clearBranch(); else EasyTccContext.bindBranch(previousBranch);
        }
    }
    ...
    private Method resolve(BranchTransaction branch, String methodName) {
        try {
            Class<?> type = Class.forName(branch.getBeanName());
            String[] names = context.getBeanNamesForType(type);
            if (names.length != 1) throw new EasyTccException("Expected exactly one Spring bean for " + type.getName() + ", found " + names.length);
            return findCompatible(AopUtils.getTargetClass(context.getBean(names[0])), methodName, branch.getArguments());
    ...
    private static Method findCompatible(Class<?> type, String name, Object[] args) {
        ...
        for (Method method : type.getMethods()) {
            ...
            for (int i = 0; i < parameters.length; i++) {
                if (args[i] != null && !wrap(parameters[i]).isInstance(args[i])) { compatible = false; break; }
                if (args[i] == null && parameters[i].isPrimitive()) { compatible = false; break; }
            }
            if (!compatible) continue;
            if (match != null) throw new EasyTccException("Ambiguous method: " + type.getName() + "#" + name);
            match = method;
        }
        if (match == null) throw new EasyTccException("Compatible method not found: " + type.getName() + "#" + name);
        return match;
    }

    private static Class<?> wrap(Class<?> type) {
        if (!type.isPrimitive()) return type;
        if (type == int.class) return Integer.class; if (type == long.class) return Long.class;
        if (type == boolean.class) return Boolean.class; if (type == byte.class) return Byte.class;
        if (type == short.class) return Short.class; if (type == char.class) return Character.class;
